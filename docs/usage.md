...
**2. Process the default video and save to a custom CSV:**
```bash
youtube-to-docs -o my-docs.csv
```

**3. Summarize a Playlist using multiple models (Gemini and Vertex):**
```bash
youtube-to-docs PLGKTTEqwhiHHWO-jdxM1KtzTbWo6h0Ycl -m gemini-3-flash-preview,vertex-claude-haiku-4-5@20251001
```

**4. Process a Channel with Summaries, TTS, and Infographics:**
```bash
...
*   **Transcript characters**: The total number of characters in the transcript.

### Files
*   **Transcript File {type}**: Path to the saved transcript file. `{type}` is either `youtube generated` or `human generated`.
