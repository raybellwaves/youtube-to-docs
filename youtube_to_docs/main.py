...
def reorder_columns(df: pl.DataFrame) -> pl.DataFrame:
    """
    Reorders the columns of a Polars DataFrame to a predefined, logical structure.

    This function organizes columns into groups:
    1.  Base metadata (URL, Title, etc.)
    2.  Transcript character counts
    3.  File paths (transcripts, summaries, etc.)
    4.  Cost columns for various AI operations
    5.  AI-generated content (speaker lists, Q&A text, summary text)

    Any columns not explicitly defined in the order are appended to the end.
    This ensures a consistent and readable CSV output.

    Args:
        df: The input Polars DataFrame.

    Returns:
        A new Polars DataFrame with columns reordered.
    """
    cols = df.columns
    base_order = [
        "URL",
...
def _process_video(
    ...
    no_youtube_summary: bool,
    languages: list[str],
    infographic_arg: str | None,
    transcripts_dir: str,
    summaries_dir: str,
    ...
    """
    Processes a single YouTube video, from fetching details to generating AI content.

    Args:
        video_id: The ID of the YouTube video to process.
        youtube_service: The YouTube API service object.
        existing_df: DataFrame of existing processed videos to avoid reprocessing.
        storage: The storage handler (e.g., LocalStorage, GoogleDriveStorage).
        transcript_arg: The source for transcription ('youtube' or an AI model).
        model_names: A list of LLM names to use for summarization and other tasks.
        no_youtube_summary: Flag to skip secondary summaries from YouTube transcripts.
        languages: A list of languages to process.
        infographic_arg: The model to use for generating infographics.
        transcripts_dir: Directory to save transcript files.
        summaries_dir: Directory to save summary files.
        infographics_dir: Directory to save infographic files.
        speakers_dir: Directory to save speaker extraction files.
        qa_dir: Directory to save Q&A files.
        audio_dir: Directory to save extracted audio files.
        local_audio_dir: Local directory for temporary audio processing.

    Returns:
        A dictionary representing the processed data for the video, ready to be
        added as a row to the final DataFrame.
    """
    ...
    for language in languages:
                ...
                if en_result:
                    ...
                    if not row.get("Transcript File human generated") and not row.get(
                        ...
                        try:
                            ...
                            if en_is_generated:
                                row["Transcript File youtube generated"] = saved_path
                            else:
        ...
        for model_name in model_names:
            ...
            qa_file_col_name = f"QA File {model_name} from {transcript_arg}{col_suffix}"
            qa_cost_col_name = (
            ...
            if (
                ...
                if yt_speakers_text == 'float("nan")' and (
                    ...
                    if yt_speakers_text and not isinstance(
                        ...
                        target_path = os.path.join(speakers_dir, yt_speakers_filename)
            ...
            if (
                ...
                yt_qa_file_col_name = f"QA File {model_name} from youtube{col_suffix}"
                yt_qa_cost_col_name = (
                    f"{normalize_model_name(model_name)} QA cost from "
                ...
                if row.get(yt_qa_file_col_name):
                    ...
                    if path:
                        try:
                            ...
                            print(
                                f"Warning: Failed to read YouTube Q&A file {path}: {e}"
                            )
            ...
            if (
                ...
                yt_sum_col_name = f"Summary Text {model_name} from youtube{col_suffix}"
...
def main(args_list: list[str] | None = None) -> None:
    """
    Main function to process YouTube videos and generate documentation.

    This function parses command-line arguments, resolves video IDs,
    processes each video to fetch transcripts, generate summaries,
    and create other artifacts, and then saves the results to a CSV file.

    Args:
        args_list: A list of command-line arguments. If None,
    ...
    for video_id in video_ids:
        row = _process_video(
            ...
            no_youtube_summary=no_youtube_summary,
            languages=languages,
            infographic_arg=infographic_arg,
            transcripts_dir=transcripts_dir,
            summaries_dir=summaries_dir,
            infographics_dir=infographics_dir,
